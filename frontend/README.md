...
Cubre, por ahora, el flujo crítico completo:

- **Login** (`/login`) — diseñado como una ficha/carnet de acceso, fiel a la identidad visual real de LA PRE PERÚ (rojo institucional, negro, plateado).
- **Escáner de asistencia** (`/escaner`, rol `personal_asistencia`) — usa la cámara del dispositivo para leer el QR con `jsQR`, llama a `POST /api/asistencia`, y muestra el resultado como un "sello" a pantalla completa (éxito / código duplicado / inválido).
- **Mi QR** (`/mi-qr`, rol `estudiante`) — genera el código QR del estudiante y muestra un anillo de cuenta regresiva hasta que expira.
- **Panel** (`/panel`, roles `direccion`/`administrador_sede`) — todavía un placeholder; los paneles de gestión son el siguiente paso.
...
Abre `http://localhost:5173`. Para el escáner necesitas otorgar permiso de cámara al navegador (en Chrome/Firefox de escritorio funciona con `localhost` sin HTTPS; en un celular real vas a necesitar HTTPS o accederlo como `localhost` vía adb/túnel para que el navegador permita la cámara).

Usa las mismas credenciales que ya probaste por curl (`asistencia_central` / `asistencia123`) para entrar directo al escáner. Para probar `/mi-qr` necesitas un usuario con rol `estudiante` y `estudiante_id` asignado — puedes crear uno igual que hiciste con `admin_central`, pero con `rol = 'estudiante'` y `estudiante_id` apuntando a un registro real de la tabla `estudiante`.
